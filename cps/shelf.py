...
def add_series_to_shelf(shelf_id, series_id):
    ...
    if not books_in_series:
        ...
        if not xhr:
            flash(_("No books found in series: %(name)s", name=series.name), category="error")
            return redirect(url_for('web.index'))
        return jsonify({'status': 'error', 'message': 'No books found in this series'}), 404
    ...
    if not books_to_add:
        log.info("All books from series %s are already in shelf %s", series.name, shelf.name)
        if not xhr:
            flash(_("All books from series '%(name)s' are already in shelf: %(shelf)s",
                   name=series.name, shelf=shelf.name), category="info")
            return redirect(url_for('web.index'))
        return jsonify({
            'status': 'info',
            'message': f"All books from series '{series.name}' are already in shelf '{shelf.name}'"
        }), 200
    ...
    try:
        ub.session.merge(shelf)
        ub.session.commit()
        log.info("Successfully added %d books from series '%s' to shelf '%s'",
                added_count, series.name, shelf.name)

        if not xhr:
            flash(_("Added %(count)d books from series '%(name)s' to shelf: %(shelf)s",
                   count=added_count, name=series.name, shelf=shelf.name), category="success")
            if "HTTP_REFERER" in request.environ:
                return redirect(request.environ["HTTP_REFERER"])
            else:
                return redirect(url_for('web.index'))
        ...
        return jsonify({
            'status': 'success',
            'message': f"Successfully added {added_count} books from series '{series.name}' to shelf '{shelf.name}'",
